...
function PageLink({ page }: { page: Page }) {
  return (
    <article key={page.href}>
      ...
      >
        <h3 className="mt-6 text-base font-semibold text-accent">
          {page.title}
        </h3>
        <time
          dateTime={page.date}
          className="order-first text-sm text-accent/60"
        >
          {formatDate(page.date)}
        </time>
        <p className="mt-2.5 text-base text-accent/60">{page.description}</p>
        <Link
          href={page.href}
          className="mt-6 flex gap-x-3 text-base font-semibold text-accent transition hover:text-accent/70"
